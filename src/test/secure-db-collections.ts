...
import { InternalCollections } from '../db/proxies/secure'
import { SecureDbItem } from '../db/proxies/secure/types'
import { DbCollections } from '../db/types'
import { createSecureMemDbLogin, getLoginBob, getRootUser, PublicThingEntityMap } from './fixtures/secure-db'
    ...
    describe('collections', () => {
      describe('ids', () => {
        ...
        it('succeeds user', async () => {
          ...
          for (const thingId of thingIds) {
            ...
            if (thingId === thingIds[0]) continue
      ...
      describe('createMany', () => {
        ...
        it('succeeds createMany users', async () => {
          ...
          const internalCollections = (
            db[InternalCollections] as
            DbCollections<PublicThingEntityMap, SecureDbItem>
          )

          const userIds = await internalCollections.user.createMany([
            getLoginBob(), { name: 'Kate', password: 'Kittens' }
          ])
      ...
      describe( 'load', () => {
        ...
        it( 'fails perm', async () => {
          ...
          const id = await db.collections.publicThing.create(
            { name: 'a', value: 0 }
          )
