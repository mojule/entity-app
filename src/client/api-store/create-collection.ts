import {
  DbIds, DbCreate, DbCreateMany, DbCollection, DbLoad, DbLoadMany, DbSave,
  DbRemove, DbRemoveMany, DbSaveMany, DbFind, DbFindOne, DbLoadPaged
} from '../../db/types'

import { kebabCase } from '../../util/lodash'
import { objToError } from '../../util/error'
import { getAuthHeaderValue } from '../utils/get-auth-header'
...
const apiPost = async <TEntityMap, K extends keyof TEntityMap>(
  ...
  let path = `/api/v1/${ kebabCase( String( key ) ) }/${ kebabCase( action ) }`
