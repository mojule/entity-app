 ...
 */
export const getZipEntries = (data: Uint8Array, dataStartOffset = 0) => {
  ...
  while (--entriesLeft >= 0 && offset < endoffset) {
    if (view.getUint32(offset) != 0x504b0102) break
    ...
    const encoding: BufferEncoding = (bitFlag & 0x800) ? 'utf-8' : 'ascii'
...
const decodeFilename = (filenameBytes: Uint8Array, encoding: BufferEncoding) =>
  new TextDecoder(encoding).decode(filenameBytes)
