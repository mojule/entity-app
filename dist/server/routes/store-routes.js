...
const defaultGetResult = async (collectionKey, store, action, type, req, omitId) => {
    ...
    log_iisnode_1.log.info('Store GET', { collectionKey, action, type });
    return handleResolve(result, store, req);
};
...
const defaultPostResult = async (collectionKey, store, action, type, req) => {
    ...
    log_iisnode_1.log.info('Store POST', { collectionKey, action, type });
    return handleResolve(result, store, req);
};
...
const handleResolve = async (result, store, req) => {
    ...
    if (resolve === 'shallow') {
        if (isEntityArray(result)) {
            return Promise.all(result.map(e => ref_1.resolveRefsShallow(store, e)));
        }
        if (isEntity(result)) {
            return ref_1.resolveRefsShallow(store, result);
        }
    }
    if (resolve === 'deep') {
        if (isEntityArray(result)) {
            return Promise.all(result.map(e => ref_1.resolveRefsDeep(store, e)));
        }
        if (isEntity(result)) {
            return ref_1.resolveRefsDeep(store, result);
        }
    }
    return result;
};
