...
const next_filename_1 = require("../util/next-filename");
...
const FileCreateStorageFactory = (db, diskFileDeps, imageFileDeps, zipFileDeps) => {
    ...
    const zipFile = async (fileData) => {
        ...
        const paths = await Promise.all(originalPaths.map(async (originalPath) => {
            ...
            const mimetype = mime.lookup(originalPath) || 'application/octet-stream';
            ...
            const getRootPath = () => path_2.join(zipFileDeps.getRootPath(fileData), dir);
...
const getPaths = (fileData, staticPath, rootPath) => {
    ...
    const destPath = path_2.join(staticPath(fileData), rootPath(fileData));
    const destName = next_filename_1.nextFilename(destPath, originalname);
    const destFilePath = path_2.join(destPath, destName);
    const uriPath = path_2.join('/', rootPath(fileData), destName);
